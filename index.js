...
// ── /pair endpoint — called by the web UI Pair button ─────────
let globalSock = null;
app.get('/pair', async (req, res) => {
  try {
    let phone = (req.query.phone || '').replace(/[^0-9]/g, '');
    if (!phone || phone.length < 7) return res.json({ error: 'Invalid phone number' });
    if (!globalSock) return res.json({ error: 'Bot not started yet. Deploy it first.' });
...
async function startBot() {
  ...
  globalSock = sock; // expose for /pair endpoint
  ...
  sock.ev.on('connection.update', async ({ connection, lastDisconnect }) => {
    ...
    if (connection === 'open') {
      ...
      // Save the connected number as owner so owner commands work for whoever paired
      if (botJid) saveOwnerJid(botJid);
      // Also update config.ownerNumber for runtime checks
      if (botJid) config.ownerNumber = botJid.replace('@s.whatsapp.net', '');
  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      try {
        ...
        // Owner = whoever connected/paired the bot (saved in owner.json)
        // Also match bot's own number and the config number as fallback
        const isOwnerMsg = senderBare === ownerJid ||
                           senderBare === botBare ||
                           getNumber(sender) === config.ownerNumber ||
                           getNumber(sender) === (ownerJid?.replace('@s.whatsapp.net','') || '');
